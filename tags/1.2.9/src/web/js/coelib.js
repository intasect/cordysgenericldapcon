...
/**
 * This method creates an XML element with the specific namespace.
 *
 * @param sTag The name of the new tag.
 * @param sNamespace The namespace of the element.
 * @param nParent The parent element.
 *
 * @return The reference to the newly created element. 
 */
function createElementNS(sTag, sNamespace, nParent)
{
	return createTextElementNS(sTag, null, sNamespace, nParent);
}
 ...
 * @return The reference to the newly created element.
 */
function createElementWithParentNS(sTag, nParent)
{
	var nReturn = null;
	...
	{
		nReturn = createTextElementNS(sTag, null, nParent.namespaceURI, nParent);
	}            
 ...
 * @return The reference to the newly created element.
 */
function createTextElementWithParentNS(sTag, sValue, nParent)
{
	var nReturn = null;            
	...
	{
		nReturn = createTextElementNS(sTag, sValue, nParent.namespaceURI, nParent);
	}            
 ...
 * @return The reference to the newly created element.
 */
function createTextElementNS(sTag, sValue, sNamespace, nParent)
{
	if (nParent == null)
	{
		alert("Parent must be filled");
	}
	else if (sTag == null || sTag == "")
	{
		alert("Tagname must be filled");
	}            
	...
	//Get the document to create the element in.
	var xmlDoc = nParent.ownerDocument;            

	var nReturn = xmlDoc.createNode(NODE_ELEMENT, sTag, sNamespace);
...
/**
 * This method defines the XPath query language for the given document and also sets the prefixes.
 *  
 * @param xmlObject The owner document.
 * @param sPrefixes The prefixes to use.
 */
function declareNamespacePrefixes(xmlObject, sPrefixes)
{
	//Make sure that either a node or a document can be passed on.
	...
	//Set XPath search language
	xmlDoc.setProperty("SelectionLanguage", "XPath");

	//Set the list of namespaces to root
	xmlDoc.setProperty("SelectionNamespaces", sPrefixes);
}

/**
 * This method creates a new XML document with the given prefix mappings defined.
 * 
 * @param sXMLString The XML string.
 * @param sPrefixes The prefixes to define on the new document.
 * 
 * @return The new document.
 */
function createXMLDocument(sXMLString, sPrefixes)
{
	var xmlReturn = new ActiveXObject("Microsoft.XMLDOM");
	xmlReturn.loadXML(sXMLString);
	
	declareNamespacePrefixes(xmlReturn, sPrefixes);
	
	return xmlReturn;
}

/**
 * This method creates a new XML document the old fashioned way.
 * 
 * @param sXMLString The XML string.
 * 
 * @return The new document.
 */
function createSimpleXMLDocument(sXMLString)
{
	var xmlReturn = new ActiveXObject("Microsoft.XMLDOM");
	xmlReturn.loadXML(sXMLString);
